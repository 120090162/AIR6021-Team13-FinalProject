...
#include <std_msgs/msg/bool.hpp> // For pick/place commands
...
private:
    ...
    rclcpp::Subscription<std_msgs::msg::Bool>::SharedPtr _pickPlaceCommandSubscriber;          ///< Subscriber for pick/place commandss
    ...
    bool _isPicking = false; ///< Used to determine if the gripper is picking up an object
    int gripperBodyId;       ///< IDs of the gripper and boxes
    double mass;
    ...
    /**
     * @brief Callback function to handle incoming pick/place commands.
     * @param msg The message containing the pick/place command.
     */
    void
    pick_place_command_callback(const std_msgs::msg::Bool::SharedPtr msg);
};
